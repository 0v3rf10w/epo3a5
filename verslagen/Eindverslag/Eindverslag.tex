...
\label{Systeemoverzicht_img}
\end{figure}

\section{Toplevel}
De toplevel beschrijving bestaat uit een entity en een behaviour. In de code van de entity staan alle poorten beschreven die de chip verbinden met de externe componenten die het systeem bevat. In de behaviour worden alle interne componenten aan elkaar verbonden. In figuur \ref{Systeemoverzicht_img} staat schematisch weergegeven op welke manier dit gebeurd. De losse onderdelen van dit systeem worden individueel beschreven in dit verslag.
...
\chapter{CPU}
\section{Functionele beschrijving}
Om programma’s uit te kunnen voeren wordt gebruik gemaakt van een door de Delta-1 geïnspireerde microprocessor. Deze 8-bits processor krijgt instructies binnen die via SPI van een SD-kaart worden afgelezen. Vervolgens voert de processor bewerkingen uit, waarvan de resultaten doorgestuurd worden naar de overige componenten op de chip. De data die uit de processor komt wordt hierna door de VGA-controller omgezet naar beelden op het scherm.
...
\item[Bits in Bytes]
	De pixelmap van het beelscherm zal in het geheugen worden als bytes, waarin elke bit een ingekleurde pixel voorsteld. Bij de logische waarde '1' zal de pixel ingekleurd worden. Zie figuur \ref{pixelmap} voor een grafische representatie van de pixelmap in bytes, en figuur \ref{byte} een representatie van de indeling van 1 individuele byte. In de pixelmap is te zien dat er exact 1 byte buiten het scherm valt in verband met de back proch van het beeldscherm.
\end{description}
...
\item [Blok H]
	Blok H schrijft naar het SRAM de instructie om te mogen schrijven en daarna enkel '0' om het volledige scherm leeg te maken.
\end{description}
...
\section{Simulatie}
Om de volledige gpu te testen is gebruik gemaalt van een testbench welke SPI reactie simuleert. in figuur \ref{gpusim} is ze zien hoe de GPU data ontvangt, vergelijkt en weer wegschrijft naar het SRAM.

\begin{figure}[H]
\center
\includegraphics[width=12cm]{gpusim}
\caption{Simulatie GPU}
\label{gpusim}
\end{figure}
...
\begin{figure}[H]
\center
\includegraphics[width=8cm]{./sd_diagram}
...
\section{Implementatie} 
Voor de implementatie van de sd-kaart module is een Finite State Machine gemaakt. Deze kan opgesplitst worden in twee delen: initializatie en het daadwerkelijke uitlezen van de data. Deze FSM is echter te comlex om in zijn geheel te beschrijven, daarom zal de FSM worden omschreven in de commando's die worden verstuurd en de voorwaarden wanneer er begonnen wordt met het sturen van het volgende commando. De initializatie verloopt volgens het diagram in figuur \ref{sd-fsm-init}, dan komt de FSM in de idle state en zal het diagram in figuur \ref{sd-fsm-read} worden doorlopen zodra er een nieuw adres is ontvangen.

\begin{figure}[H]
\centering
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[width=6cm]{./SD_FSM_init}
  \captionof{figure}{Finite state diagram van SD-kaart initializatie}
  \label{sd-fsm-init}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[width=5cm]{./SD_FSM_read}
  \captionof{figure}{Finite state diagram van het lezen van de SD-kaart} 
  \label{sd-fsm-read}
\end{minipage}
\end{figure}
...
\section{Conclusie}
Het EPO-3 project van de bachelor opleiding Electrical Engineering begon in tegen stelling tot voorgaande projecten vrij soepel. Als groep waren we het snel met elkaar eens, het zou een spelcomputer worden met een ultrasone besturing. Gaande weg liepen we tegen steeds meer complicaties aan. \\


\subsection{Complicaties}
Gedurende de uitvoering van het project is gebleken dat er een aantal problemen zijn ontstaan met betrekking tot de communicatie met externe geheugenmodules. Zo is de communicatie met de SD kaart niet op gang gekomen wegens een incorrecte VHDL code. Tevens het SRAM heeft op geen enkele manier gereageerd op de SPI signalen. Mogelijke oorzaken van het niet functioneren van het SRAM en de SD-kaart zouden kunnen liggen bij het uitgangssignaal van de FPGA. In figuur \ref{kutbeeld} is een foto te zien met daarop de uitgangssignalen van de FPGA. Deze signalen vertonen duidelijk geen correcte blokvorm. Voor onderzoek naar het oplossen van dit probleem bleek te weinig tijd te zijn.

\begin{figure}[H]
\center
\includegraphics[width=5cm]{scope}
\caption{Uitgang FPGA}
\label{kutbeeld}
\end{figure}

\subsection{Resultaten}
Door het wegvallen van het SRAM en de SD-kaart is besloten om de volledige VGA output te laten vallen, tevens de externe ROM (in de vorm van de SD kaart) is niet gebruikt bij het uiteindelijke design. Er is besloten om met de werkende componenten een rekenmachine te maken. Deze zal zijn ingang ontvangen via een externe microcontroller over SPI. Er zal gebruik worden gemaakt van de 8-bit CPU en on-board ROM. In de bijlage is de gebruikte assambly code te vinden welke de operaties optellen, aftrekken en vermenigvuldigen realiseert.
...
EPO-3 groep A5 begon dit project sterk en georganiseerd, dit was grotendeels te danken aan onze voorzitter Erné Bronkhorst. Beslissingen konden gezamenlijk en snel worden genomen, dit gaf ons een goede voorsprong. Ook de taakverdeling en planning leek erg soepel te lopen. Gaande weg het project werd steeds meer duidelijk dat niet alle onderdelen evenveel tijd kosten, dit had tot gevolg dat er achteraf in moest worden gesprongen in onderdelen en dat koste erg veel tijd. De besturing was daar een goed voorbeeld van, deze kon eigenlijk vrij snel gerealiseerd worden. Hierdoor kwamen er twee mensen zonder taak te zitten, gelukkig kon dit vrij snel worden opgelost. Er moest nog een plan van aanpak worden geschreven en een toplevel beschrijving ontworpen. In de laatste fase van het project heeft de taakverdeling ervoor gezorgd dat we dit toch nog met een goed resultaat konden beëindigen. 
 ...
 \begin{thebibliography}{2}
  ...
  \bibitem{SanDisk} SanDisk Secure Digital Card Product Manual Version 1.9, http://alumni.cs.ucr.edu/~amitra/sdcard/ProdManualSDCardv1.9.pdf, benaderd in november en december. 2014.
...
\section{SD-kaart}
\label{SD_kaart}
\lstinputlisting[language=VHDL]{VHDL/sdcard.vhd}
...
\lstinputlisting[language=VHDL]{VHDL/reg_8.vhd.vhd}
\lstinputlisting[language=VHDL]{VHDL/reg_9.vhd}
...
\newpage
\section{VGA}
\lstinputlisting[language=VHDL]{VHDL/VGA/counter-behaviour.vhd}
\lstinputlisting[language=VHDL]{VHDL/VGA/deler-behaviour.vhd}
\lstinputlisting[language=VHDL]{VHDL/VGA/hsync-behaviour.vhd}
\lstinputlisting[language=VHDL]{VHDL/VGA/rgb-behaviour.vhd}
\lstinputlisting[language=VHDL]{VHDL/VGA/scount-behaviour.vhd}
\lstinputlisting[language=VHDL]{VHDL/VGA/ssync-behaviour.vhd}
\lstinputlisting[language=VHDL]{VHDL/VGA/verticaltel.vhd}
\lstinputlisting[language=VHDL]{VHDL/VGA/vga-behaviour.vhd}
\lstinputlisting[language=VHDL]{VHDL/VGA/vidcounter-behaviour.vhd}
\lstinputlisting[language=VHDL]{VHDL/VGA/vsync-behaviour.vhd}

\section{Assembly}
\lstinputlisting{assambly.md}
